# File: test_SI_barriere.py
# Run as: python bpm.py test_SI_barriere 1D
#
...
def psi_0(x, y):
    ...
    barrier_left = 1.0 - depth_barrier * np.exp(-(x + d_well)**2 / (2 * width_barrier**2))
    barrier_right = 1.0 - depth_barrier * np.exp(-(x - d_well)**2 / (2 * width_barrier**2))

    # Séparation spatiale initiale
    psi1 = np.sqrt(n_TF_a) * (x < 0) * barrier_left * (x < 0)
    psi2 = np.sqrt(n_TF_b) * (x > 0) * barrier_right * (x > 0)
