...
if module_name == "1D":
    module_name = "1D_unique"
build = importlib.__import__(module_name)	     # selects 1D or 2D
...
def normalize_density(psi):
    """Normalize the density |psi|^2 between 0 and 1."""
    density = np.abs(psi)**2
    d_min = density.min()
    d_max = density.max()
    if d_max - d_min < 1e-15:   # avoid division by zero if density is uniform
...
for j in range(steps_image*my.images+1):		# propagation loop
	if j%steps_image == 0:  # Generates image output
		density_norm = normalize_density(psi)   # normalized density in [0, 1]
